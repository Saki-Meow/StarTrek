...
let humanFemale = humanOfficers.filter{femaleOfficers.contains($0)}
print(Float(humanFemale.count) / Float(humanOfficers.count))



// Q2: How many first officers are human?
let humanNum = firstOfficers.keys.filter{humanOfficers.contains($0)}
print(humanNum)
...
let firstVulcanOfficers: [Officer] = firstOfficers.map{Officer(name: $0, ship: $1)}.filter{vulcanOfficers.contains($0.name)}
print(firstVulcanOfficers.map{$0.name})



// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let shortFirstVulcanNum = firstVulcanOfficers.reduce(0) {$1.name.count < 5 ? $0 + 1 : $0}
print(shortFirstVulcanNum)
